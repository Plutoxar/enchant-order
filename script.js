...
window.onload = async function () {
    ...
    await (async () => {
        const module = await import(`./data.js?2`);
        data = module.data;
    ...
    if (load) {
        await (async () => {
            ...
            if (modpackName === 'Minecraft') {
                module = await import(`./data.js?2`);
            }
            else {
                module = await import(`./modpacks/${modpackName}/data.js`);
            }
            data = module.data;
...
async function buildLanguage(lang = 'en', update = false) {
    ...
    extraInfo.href = 'https://github.com/Plutoxar/enchant-order-solver'
...
function getText(value, type = 'time') {
    ...
    if (languageJson?.level_s) {
        pluralSuffix = value > 1 || value === 0 ? '_s' : '';
    } else {
        pluralSuffix = (value % 10 > 4 || value % 10 === 0) ? '_high' : (value % 10 > 1) ? '_low' : '';
    }
    // Unit type
    const unit = (type === 'time' && value > 2000) ? ['second', Math.round(value / 1000)] : [type === 'time' ? 'millisecond' : 'level', Math.round(value)];
